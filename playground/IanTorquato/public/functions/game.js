export default function createGame() {
  ...
  function start() { setInterval(() => addFruit({ fruitId: 0, fruitX: 0, fruitY: 0 }), 2000) }
  ...
  function movePlayer(command) {
    ...
    const acceptedMoves = {
      ...
      s(player) { player.y = Math.min(player.y + 1, state.screen.height) },

      // Mover para a direita
      ArrowRight(player) { player.x = Math.min(player.x + 1, state.screen.width) },

      d(player) { player.x = Math.min(player.x + 1, state.screen.width) },
  ...
  return { start, subscribe, setState, addPlayer, removePlayer, movePlayer, addFruit, removeFruit, state }
}
